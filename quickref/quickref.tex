...
{\small \textbf{Try[T]} is (also) like a one element collection. \textbf{Try[T]} is either \textbf{Success[T]} (value present) or \textbf{Failure[E]} (where E is an exception). Try, Success, Failure needs import: \code|import scala.util.{Try, Success, Failure}|} \\
% {\small \textbf{Option[T]} is like a collection with zero or one element. \textbf{Some[T]} and \textbf{None} are subtypes of Option. }
{\small Rule of thumb: collection-like methods work on both \textbf{Option} and \textbf{Try[T]}, e.g. foreach, map, filter, toVector, ...} \\
...
\multicolumn{2}{l}{\hspace{-0.62em}Accessing values for \code|val maybe: Option[String]| or \code|val maybe: Try[String]|} \\

\code|maybe.get|
& \Comment{if maybe == Some[T] / Success[T] then x else throws NoSuchElementException}\\

\code|maybe.getOrElse(expr)|
& \Comment{if opt == Some[T] / Success[T] then x else expr}\\

\code|maybe match { case Some(x) / Success(x) => expr1;  case None / e => expr2} |
